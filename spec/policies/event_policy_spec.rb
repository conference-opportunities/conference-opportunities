  ...
  describe EventPolicy::Scope do
    context 'when not logged in' do
      context 'when there are no conferences' do
        specify { expect(EventPolicy::Scope.new(nil, Event).resolve).to eq([]) }
      end
      ...
      context 'when there is a conference' do
        ...
        context 'when the conference is followed and approved'do
          ...
          specify { expect(EventPolicy::Scope.new(nil, Event).resolve).to eq([event]) }
        end
      end
    end
    ...
    context 'when logged in as the admin' do
      ...
      context 'when the conference is not approved' do
        ...
        specify { expect(EventPolicy::Scope.new(user, Event).resolve).to eq([event]) }
      end
    ...
    context 'when logged in as the organizer of the conference' do
      ...
      context 'when the conference is not approved' do
        ...
        specify { expect(EventPolicy::Scope.new(user, Event).resolve).to eq([]) }
      end
